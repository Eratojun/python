...
if local_ip == "192.168.0.27":  # 사무실에서 사용하는 IP
    file_path30 = r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20251111금상동.xlsx'  # 엑셀 파일 경로를 지정하세요
elif local_ip != "192.168.0.27":  # 사무실이 아닌경우 IP
    file_path30 =r'C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20251111금상동.xlsx'
else:  # 기본 경로 (기타 환경)
...
#file_path30 = r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20241128금상동.xlsx'  # 엑셀 파일 경로를 지정하세요
#file_path30 =r'C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20241128금상동.xlsx'
             

if local_ip == "192.168.0.27":  # 사무실에서 사용하는 IP
    file_path30 = r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20251111금상동.xlsx'  # 엑셀 파일 경로를 지정하세요

    output_file = r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20251111금상동(관리선수금월수).xlsx'
elif local_ip != "192.168.0.27":  # 사무실이 아닌경우 IP
    file_path30 =r'C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20251111금상동.xlsx'
    output_file = r'C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20251111금상동(관리선수금월수).xlsx'
else:  # 기본 경로 (기타 환경)
    file_path30 = "/mnt/default_path/"
...
#output_file = r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20241128금상동(관리선수금월수).xlsx'
...
df30['적요1'] = 0.0 #일반관리기간을 담기위한 칼럼
df30['적요2'] = 0.0 #회전관리기간을 담기위한 칼럼
# 사용자로부터 연도 입력받기
...
if input_year == input_year:
    ...
    new_column_name4 = "적요1"
    df30['계약일'] = df30['계약일'].astype(str)
    df30['봉안일1'] = df30['봉안일1'].astype(str)
    df30['봉안일2'] = df30['봉안일2'].astype(str)
    ...
    mask_a = (df30['계약일'].str.contains(input_year, na=False) & \
           (df30['봉안일1'].isna()) & \
           (df30['봉안일2'].isna()) & \
           (df30['관리시작'].isna()) & \
           (df30['관리종료'].isna()) 
    ...
    mask2_b = (
    contains_year(df30['계약일'], input_year) &  # 첫 번째 조건
    contains_year(df30['봉안일1'], input_year) &  # 두 번째 조건
    df30['봉안일2'].isna() &  # 세 번째 조건
    (df30['관리수입'] > 0)  # 네 번째 조건
    ) 
    ...
    mask2_c = (
    df30['계약일'].str.contains(input_year, na=False) &  # '계약일'이 input_year 포함
    ~df30['봉안일1'].str.contains(input_year, na=False) &  # '봉안일1'이 input_year 포함 X
    df30['봉안일2'].isna()  # '봉안일2'가 결측치
    )


    mask3_d = (contains_year(df30['계약일'], input_year)) & \
              (contains_year(df30['봉안일2'], input_year)) & \
              (df30['봉안일1'].isna()
    )  


    mask3_d1 = (contains_year(df30['계약일'], input_year)) & \
              (contains_year(df30['봉안일2'], input_year)) & \
              (df30['봉안일1'].isna() & \
               (df30['관리시작'].isna()) & \
               (df30['관리종료'].isna())
    ...
    mask3_e = (
    df30['계약일'].str.contains(input_year, na=False) &  # '계약일'이 input_year 포함
    ~df30['봉안일2'].str.contains(input_year, na=False) &  # '봉안일1'이 input_year 포함 X
    df30['봉안일1'].isna()  # '봉안일2'가 결측치
    )

    mask4_f = (
    df30['계약일'].str.contains(input_year, na=False) &  # '계약일'이 input_year 포함
    df30['봉안일1'].str.contains(input_year, na=False) &
    df30['봉안일2'].str.contains(input_year, na=False)   # '봉안일1'이 input_year 포함 X
    )
    ...
    mask5_j = (
    ~df30['계약일'].str.contains(input_year, na=False) &  # '계약일'이 input_year 불불포함
    df30['봉안일1'].str.contains(input_year, na=False) &
    df30['봉안일2'].str.contains(input_year, na=False)   # '봉안일1'이 input_year 포함 X
    )
    ...
    ~df30['계약일'].str.contains(input_year, na=False) &  # '계약일'이 input_year 불불포함
    df30['봉안일1'].str.contains(input_year, na=False) &
    ~df30['봉안일2'].str.contains(input_year, na=False) &
    ~df30['최종입금일'].str.contains(input_year, na=False)   # '봉안일1'이 input_year 포함 X
    )
    ...
    # '계약일' 열에 입력받은 연도를 포함하고, 나머지 열이 모두 결측치일 경우 마스크 정의
    mask6 = (contains_year(df30['계약일'], input_year)) & \
           (df30['봉안일1'].isna()) & \
           (df30['봉안일2'].isna()) & \
           (df30['관리시작'].isna()) & \
           (df30['관리종료'].isna())
    ...
    #mask2 = (contains_year(df30['관리시작'], input_year)) & \
            #(df30['사용수입'] > 0)
    # 여러 마스크를 결합 (mask2 OR mask3 OR mask4)
    #combined_mask = mask2 | mask3 | mask4
    # 계약일이 2024년인 사용수입인식식
    #combined_mask = (mask2_b | mask3_d |	mask3_d1 |	mask3_e | mask4_f 	| mask4_g |	mask4_h | mask4_i |	mask5_j	| mask5_k | mask5_K1 |	mask5_l	| mask5_m) & (df30['사용수입'] > 0)#사용수입인식 2024 ,2024=\
    combined_mask = (mask2_b | mask3_d |	mask3_d1 |	mask3_e | mask4_f 	| mask4_g |	mask4_h | mask4_i |	mask5_j	| mask5_k | mask5_K1 |	mask5_l	| mask5_m) & (df30['사용수입'] > 0)
    combined_mask2 = (mask2_b | mask3_d |	mask3_d1 |	mask3_e | mask4_f 	| mask4_g |	mask4_h | mask4_i |	mask5_j	| mask5_k | mask5_K1 |	mask5_l	| mask5_m) & (df30['관리수입'] > 0)
    combined_mask3 = (mask_a | mask2_b | mask3_d |	mask3_d1 |	mask3_e | mask4_f 	| mask4_g |	mask4_h | mask4_i ) & (df30['사용수입'] > 0)
    #combined_mask6 = (mask6 ) & (df30['사용선수'] > 0)
    combined_mask6 = (mask_a | mask2_b | mask3_d |	mask3_d1 |	mask3_e | mask4_f 	| mask4_g |	mask4_h | mask4_i |	mask5_j	| mask5_k | mask5_K1 |	mask5_l	| mask5_m) & (df30['사용선수'] > 0)
    combined_mask7 = ( mask2_b | mask3_d |	mask3_d1 |	mask3_e | mask4_f 	| mask4_g |	mask4_h | mask4_i |	mask5_j	| mask5_k | mask5_K1 |	mask5_l	| mask5_m) & (df30['사용수입'] > 0)
    #


# 조건에 따른 값 설정
    df30.loc[combined_mask, new_column_name3] = df30.loc[combined_mask, '사용수입']
    #df30.loc[combined_mask2, new_column_name3] = df30.loc[combined_mask2, '관리수입']

    # '사용선수' 값이 있는 행에서 '2023-사용선수금' 열에 값을 넣음
    #df30.loc[mask_a, new_column_name] = df30.loc[mask_a, '사용선수']
    df30.loc[combined_mask2, new_column_name2] = df30.loc[combined_mask2,'관리수입']
    
    df30.loc[combined_mask7, new_column_name] = df30.loc[combined_mask7, '사용수입']
    df30.loc[combined_mask6, new_column_name] = df30.loc[combined_mask6, '사용선수']
...
#"C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동월별결산부"
local_ip = get_local_ip()

if local_ip == "192.168.0.27":  # 사무실에서 사용하는 IP
    file_path30 = r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_20251111금상동.xlsx'  # 엑셀 파일 경로를 지정하세요
    #input_folder=r"C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동월별결산부"
    ...
    #input_folder=r"C:\dev\myproj01\금상동2024\금상동2025-2"
elif local_ip != "192.168.0.27":  # 사무실이 아닌경우 IP
    #input_folder=r"C:\dev\myproj01\금상동2024"
    #input_folder=r"C:\dev\myproj01\금상동2024\금상동2025-2"
    #input_folder=r"C:\Users\erato2022\OneDrive\삼례토지\사진\바탕 화면\금상동2024\xlsx"
    input_folder=r"C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동월별결산부"   
        #input_folder=r"C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동월별결산부"
    #file_path30 =r'C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\통합조회_202511112금상동.xlsx'
...
excel_files=raw_data_dir.glob('25년*')
...
total_df10=pd.DataFrame() #빈 데이터프래임 생성
...
for excel_file in excel_files:
    ...
    if len(df.columns) == 7:
     df['적요2'] = None
     #df = df[['날짜', '성명', '봉안함', '적요1', '적요2', '차변', '대변', '잔액']]
    df.columns = ['날짜', '성명', '봉안함','적요1','적요2', '차변', '대변', '잔액']
    #df['날짜'] = pd.to_datetime(df['날짜']).dt.strftime('%Y-%m-%d')
    
    df2 =pd.read_excel(excel_file,sheet_name="일반관리선수-최준철세무사",skiprows=5,usecols="a:g")
    if len(df2.columns) == 7:
     df2['적요2'] = None
    df2.columns = ['날짜', '성명', '봉안함', '적요1','적요2', '차변', '대변', '잔액']
    ...
    if len(df3.columns) == 7:
     df3['적요2'] = None
    df3.columns = ['날짜', '성명', '봉안함', '적요1','적요2', '차변', '대변', '잔액']
    #df3['날짜'] = pd.to_datetime(df3['날짜']).dt.strftime('%Y-%m-%d')
    
    df10= pd.read_excel(excel_file,sheet_name="납골함수입",skiprows=4,usecols="a:g")
    if len(df10.columns) == 7:
     df10['적요2'] = None
    df10.columns = ['날짜', '성명', '봉안함', '적요1', '적요2','차변', '대변','잔액']
     ...
     # 엑셀 파일의 시트 목록을 가져옴
        ...
        if "회전관선" in xls.sheet_names:
          ...
          if len(df4.columns) == 7:
           df2['적요4'] = None
          df4.columns = ['날짜', '성명', '봉안함','적요1','적요2', '차변', '대변', '잔액']
    ...
    df5.columns = ['월일', '순번', '적       요', '계정과목', '납부방법', '봉안실', '회계계정','수입','지출']
    # 엑셀 시트 이름 목록 확인
   ...
   #  print(f"시트 '{sheet_name}'가 존재하지 않습니다.")
    ...
    total_df10=total_df10._append(df10,ignore_index=True)
...
total_df10['날짜']=total_df10['날짜'].ffill() # 최근일 월일없는행은 전일자로 처리
...
#total_df10= total_df10.dropna(subset=["봉안함"]) #아래 누적액,월계액을 없애기위해 회계계정없는부분을 삭제함
# '월일' 열을 날짜(datetime) 형식으로 변환
total_df5['월일'] = pd.to_datetime(total_df5['월일'], errors='coerce')
total_df10['날짜'] = pd.to_datetime(total_df10['날짜'], errors='coerce')
total_df10['날짜'] = total_df10['날짜'].dt.strftime('%Y-%m-%d')
...
#total_df10['날짜']=total_df10['날짜'].dt.strftime('%Y-%m-%d') #구매자결제일이 날짜함수이므로 문자함수로 변환

#total_df10['날짜']=total_df10['날짜'].str.replace('-','')
...
total_df10.columns = total_df10.columns.str.strip()

#total_df5= total_df5.dropna(subset=["적       요"]) #아래 봉안함번로 없는 행을 없애기위해 봉안함없는 부분을 삭제함
total_df5["적       요"]=total_df5["적       요"].ffill() 
total_df5['월일']=total_df5['월일'].ffill()
...
total_df10= total_df10.dropna(subset=["봉안함"]) #아래 봉안함번로 없는 행을 없애기위해 봉안함없는 부분을 삭제함




total_df.columns = ['날짜','성명','봉안함','적요1','적요2','차변','대변','잔액']
total_df2.columns = ['날짜','성명','봉안함','적요1','적요2','차변','대변','잔액']
total_df3.columns = ['날짜','성명','봉안함','적요1','적요2','차변','대변','잔액']
total_df4.columns = ['날짜', '성명','봉안함','적요1','적요2','차변','대변','잔액']
total_df10.columns = ['날짜', '성명','봉안함','적요1','적요2','차변','대변','잔액']
...
total_df10['날짜'] = total_df10['날짜'].ffill()
total_df10=total_df10.sort_values(["봉안함"]) # 일자와 환율을 일자와 환율로 정렬
#total_df11=total_df10.sort_values(["날짜"]) # 일자와 환율을 일자와 환율로 정렬




#total_df = pd.concat([total_df, df1])
total_df.to_excel("C:\dev\myproj01\금상동\결과폴더\사용선수금2025.xlsx")
total_df2.to_excel("C:\dev\myproj01\금상동\결과폴더\일반관리선수-최준철세무사2025.xlsx")
total_df3.to_excel("C:\dev\myproj01\금상동\결과폴더\사용료수입2025.xlsx")
total_df4.to_excel("C:\dev\myproj01\금상동\결과폴더\회전일반관리선수-최준철세무사2025.xlsx")
total_df5.to_excel("C:\dev\myproj01\금상동\결과폴더\월별결산부2025.xlsx") 
total_df10.to_excel("C:\dev\myproj01\금상동\결과폴더\납골함수입2025.xlsx") 
#########################################################################################
# 월별결산부에서 관리선수금과 회전관리선수금을  봉안실로 검토해서  관리선수금, 회전관리선수금으로  회계계정을 
#  변경하는 작업  금상동에서 관리수입으로 잡은후  회전관리선수금으로 이체하기때문에 
#파이썬 코딩에서 total_df5 데이터프레임을 차례대로 불러와서 불러온 i행의 회계계정이 관리수입이거나 관리선수이고 
#다음행i+1 행에 같은 봉안실번호가 존재하고 회계계정칼럼에 '회전관리선수' 단어를 포함하면 i행 total_df5['코드1']값에 275값을 저장하고 
#i+2행이후의 행에 회계계정칼럼에 '일반관리선수' 단어를 포함하면 i행 total_df5['코드1']값에 274값을 저장하고 
#다음행 차례대로불어와 이 작업을 반복한 후 새로 변경된 데이터프레임을 total_df55에 저장하는 파이썬코딩보여줘


total_df5 = total_df5.reset_index(drop=True)
current_room = total_df5.loc[i, '봉안실']

# 각 행 반복
for i in range(len(total_df5) - 1):  # i+1까지 확인하므로 마지막-1까지만
    # 1) i행이 관리수입인지 확인
    if str(total_df5.loc[i, '회계계정']) in ['관리수입', '관리선수']:
        current_room = total_df5.loc[i, '봉안실']
        
        # 2) i+1행 확인
        if total_df5.loc[i + 1, '봉안실'] == current_room:
            if '회전관리선수' in str(total_df5.loc[i + 1, '회계계정']):
                total_df5.loc[i, '회계계정'] = "회전관리선수금"
            elif '일반관리선수' in str(total_df5.loc[i + 1, '회계계정']):
                total_df5.loc[i, '회계계정'] = "관리선수금"
        # 3) i+2행 이후 확인 (같은 봉안실번호에서 일반관리선수 찾기)
        mask = (total_df5.index > i + 1) & (total_df5['봉안실'] == current_room)
        next_rows = total_df5[mask]
        if not next_rows.empty:
            if next_rows['회계계정'].astype(str).str.contains('일반관리선수').any():
                total_df5.loc[i, '회계계정'] = "관리선수금"
...
# 새로운 데이터프레임에 저장
total_df5 = total_df5.copy()
total_df5.to_excel("C:\dev\myproj01\금상동\결과폴더\월별결산부2025-2.xlsx") 
...
if local_ip == "192.168.0.27":  # 사무실에서 사용하는 IP
    file_path31 =r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동계정코드(2024-11-21).xlsx' 
elif local_ip != "192.168.0.27":  # 사무실이 아닌경우 IP
    file_path31 =r'C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동계정코드(2024-11-21).xlsx' 
else:  # 기본 경로 (기타 환경)
    file_path30 = "/mnt/default_path/"
...
def calculate_gubun(row):
    ...
    if  ("이체" in E and "관리수입" in G and H < 10000) : #관리수입중 1만이상인 경우만 인식하기
        return 0 
    elif "현금" in E and "관리수입" in G and H > 0 :
            return 2
    elif "현금" in E and "사용수입" in G and H > 0 :
            return 2   
    elif "현금" in E and "봉안함" in G and H > 0 :
            return 2       
    elif "현금" in E and H > 0:
        return 2
    elif "신용" in E and "관리선수" in G and H > 0 :
        return 4     
    elif ("대체" in E  and "해약" in D and "사용수입" in G) and I > 0:
        return 4
    elif ("대체" in E  and "해약" in D and "사용선수" in G) and I > 0:
        return 4
    elif ("대체" in E  and "해약" in D and "관리선수" in G) and I > 0:
        return 4

    elif ("이체" in E and "해약" in D) and I > 0:
        return 4
    elif (
        ("이체" in E and H > 0) or
        ("이체" in E and "일반관리선수" in G and I > 0) or
        ("이체" in E and "회전관리선수" in G and I > 0) or
        ("이체" in E and "사용수입" in G and I > 0) or
        ("이체" in E and "봉안함" in G and H > 0) or
        ...
        ("신용" in E and "관리선수" in G and H > 0) or
        ("신용" in E and "봉안함" in G and H > 0)  or
        ("신용" in E and "관리수입" in G and H > 0) or
        ("신용" in E and "관리선수금" in G and H > 0) or
        ("신용" in E and "사용수입" in G and H > 0)
        ):
        return 4
    elif "신용" in E and "관리선수" in G and H > 0 :
       return 4 
...
for index, row in mon_df.iterrows():
    ...
    # 두번째 조건
    elif "해약" in str(row['계정과목']) and row['납부방법'] == "이체" and (row['회계계정'] in ["관리선수"]) and row['지출'] > 0:
        mon_df.at[index, '코드1'] = 274
    elif "해약" in str(row['계정과목']) and row['납부방법'] == "이체" and row['회계계정'] == "사용수입" and row['지출'] > 0:
        mon_df.at[index, '코드1'] = 103  
    elif "해약" in str(row['계정과목']) and row['납부방법'] == "이체" and row['회계계정'] == "관리선수금" and row['지출'] > 0:
        mon_df.at[index, '코드1'] = 103       
    elif "해약" in str(row['계정과목']) and row['납부방법'] == "대체" and row['회계계정'] == "사용수입" and row['지출'] > 0:
        mon_df.at[index, '코드1'] = 103
    elif "해약" in str(row['계정과목']) and row['납부방법'] == "대체" and row['회계계정'] == "사용선수" and row['지출'] > 0:
        mon_df.at[index, '코드1'] = 103   
    elif "해약" in str(row['계정과목']) and row['납부방법'] == "대체" and row['회계계정'] == "관리선수" and row['지출'] > 0:
        mon_df.at[index, '코드1'] = 103
    ...
    elif "대체" in str(row['납부방법']) and "관리선수금" in str(row['회계계정']) and row['수입'] > 0:
        mon_df.at[index, '코드1'] = 0    
    
    elif "신용" in str(row['납부방법']) and "사용선수" in str(row['회계계정']) and row['수입'] > 0:
        mon_df.at[index, '코드1'] = 273

    elif "신용" in str(row['납부방법']) and "사용수입" in str(row['회계계정']) and row['수입'] > 0:
        mon_df.at[index, '코드1'] = 273  
      
       # 여섯번째 조건
    elif "신용" in str(row['납부방법']) and row['회계계정'] == "관리수입" and row['수입'] > 0:
        mon_df.at[index, '코드1'] = 274
    elif "신용" in str(row['납부방법']) and row['회계계정'] == "봉안함" and row['수입'] > 0:
        mon_df.at[index, '코드1'] = 422   
      
    elif "현금" in str(row['납부방법']) and row['회계계정'] == "관리수입" and row['수입'] > 0:
        mon_df.at[index, '코드1'] = 274
    elif "이체" in str(row['납부방법']) and row['회계계정'] == "봉안함" and row['수입'] > 0:
        mon_df.at[index, '코드1'] = 422   
    
    
    elif "이체" in str(row['납부방법']) and row['회계계정'] == "관리수입":
        mon_df.at[index, '코드1'] = 274
    elif "이체" in str(row['납부방법']) and row['회계계정'] == "사용수입":
        mon_df.at[index, '코드1'] = 273
...
merged_df.to_excel('C:\dev\myproj01\금상동\결과폴더\월별병합엑셀파일08-19.xlsx', index=False)
...
# 3. 병합된 결과를 엑셀 파일로 저장
merged_df.to_excel('C:\dev\myproj01\금상동\결과폴더\월별병합코드엑셀파일08-19.xlsx', index=False)
...
def calculate_code2(row):
    ...
    if row['지출'] > 0:
        if '이체' in 납부방법:
            if '일반관리' in 회계계정:
                ...
                return 275
            elif '관리선수' in 회계계정 and '해약' in 계정과목 :
                return 103
    ...
    if row['수입'] > 0:
        ...
        elif '대체' in 납부방법 and '사용수입' in 회계계정 :
            return 273  # C50 외상매출금 참조
...
if local_ip == "192.168.0.27":  # 사무실에서 사용하는 IP
    file_path32 =r'C:\Users\erato2022\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동계정코드(2024-11-21).xlsx' 
elif local_ip != "192.168.0.27":  # 사무실이 아닌경우 IP
    file_path32 =r'C:\Users\erato\OneDrive\문서\2.금상동천호성지\2025기초데이터\금상동계정코드(2024-11-21).xlsx' 
else:  # 기본 경로 (기타 환경)
    file_path30 = "/mnt/default_path/"
...
merged_df['번호']=merged_df['순번']


print(merged_df)
#merged_df.info()

merged_df.to_excel('C:\dev\myproj01\금상동\결과폴더\금상동월별결산부2025업로드20.xlsx', index=False)
...
#year=int({input_year})-1

year = int(input_year) - 1
 # 엑셀 파일 불러오기
#file_path3 = r"C:\dev\myproj01\금상동세무사랑회전관리선수금2022-2024.xls"   # 엑셀 파일 경로를 지정하세요
file_path3= f"C:\dev\myproj01\금상동\결과폴더\회전일반관리선수-최준철세무사2022-{year}.xlsx"

df17= pd.read_excel(file_path3,sheet_name="Sheet1",skiprows=0,usecols="a:h") 
...
# df17과 total_df4를 '봉안함'을 기준으로 병합
#df20 = pd.merge(df17, total_df4, on="봉안함", how="outer")

# 차변 + 대변을 잔액 열에 저장
#df20["잔액"] = df20["차변"] + df20["대변"]
...
df20["적요2"] = df20["적요1"]
df20["적요1"] = 0
#df20["차변"] = df17["차변"] + total_df4["차변"]
#df20["대변"] = df17["대변"] + total_df4["대변"]
df20.to_excel(f"C:\\dev\\myproj01\\금상동\\결과폴더\\회전관리선수통합-{input_year}.xlsx")
df20.to_excel(f"C:\\dev\\myproj01\\금상동\\결과폴더\\회전관리선수-최준철세무사2022-{input_year}.xlsx")
...
# 데이터프레임과 시트 이름 리스트
dfs = [merged_df,total_df, total_df2,total_df3,total_df10,total_df4,df20,total_df6,total_df7,total_df8,total_df9]
...
sheet_names = ['월별결산부','사용선수금', '일반관리선수-최준철세무사','사용료수입','납골함수입','회전관리선수금','회전관리선수금통합-봉안함순','사용선수금-일자순','일반관리선수-최준철세무사-일자순','사용료수입-일자순','회전관리선수금-일자순']
# 함수 호출
...
sheet_names = ['사용선수금', '일반관리선수-최준철세무사','사용료수입','납골함수입','회전관리선수금','회전관리선수금통합-봉안함순','월별결산부','사용선수금-일자순','일반관리선수-최준철세무사-일자순','사용료수입-일자순','회전관리선수금-일자순']  # 작업할 시트 이름들
...
df_a = df_a.drop(columns=["적요1", "적요2"], errors="ignore")
...
df_a1=df_a1.rename(columns={"대변_b6":"누적-회전관리선수금"})
df_a1=df_a1.rename(columns={"대변_b5":"2025-회전관리선수금"})
df_a1=df_a1.rename(columns={"차변_b2":"사용료수입"})

df_a1['봉안일1'] = pd.to_datetime(df_a1['봉안일1'], errors='coerce')
df_a1['봉안일2'] = pd.to_datetime(df_a1['봉안일2'], errors='coerce')
df_a1['만기일1'] = pd.to_datetime(df_a1['만기일1'], errors='coerce')
df_a1['만기일2'] = pd.to_datetime(df_a1['만기일2'], errors='coerce')
df_a1['관리시작'] = pd.to_datetime(df_a1['관리시작'], errors='coerce')
df_a1['관리종료'] = pd.to_datetime(df_a1['관리종료'], errors='coerce')
...
df_a1.loc[mask3, '연중관리수입2'] = np.where(
    df_a1.loc[mask3, '적요1'] > 0,
    (df_a1.loc[mask3, '관리수입'] / (df_a1.loc[mask3, '적요1'] * 12)) * df_a1.loc[mask3, '년중월수'],
    (df_a1.loc[mask3, '관리수입'] / df_a1.loc[mask3, '관리선수금전체월수']) * df_a1.loc[mask3, '년중월수']
)
...
# '봉안함' 컬럼을 기준으로 중복 제거하고 df_a에 다시 저장
df_a = df_a1.drop_duplicates(subset=['봉안함'], keep='first')
...
# 모든 시트에 대해 A열부터 D열까지의 너비를 25로 설정
columns_to_resize = ['A', 'B', 'C', 'D','E','F','G','h','Y', 'Z','AA', 'AB','AC','AD','AH', 'AI','AJ', 'AK','AL','AM','AN']  # A열부터 D열까지 지정
...
# '봉안함' 컬럼을 기준으로 중복 제거하고 df_a에 다시 저장
df_a = df_a.drop_duplicates(subset=['봉안함'], keep='first')
...
df28 = df37[(df37['계정코드2'] == '관리선수금') & (df37['금액'] > 0)]
df29 = df37[(df37['계정코드2'] == '사용선수금') & (df37['금액'] > 0)]
df30 = df37[(df37['계정코드2'] == '회전관리선수금') & (df37['금액'] > 0)]
df31 = df37[(df37['계정코드2'] == '납골함수입') & (df37['금액'] > 0)]
...
df_grouped5 = df31.groupby('봉안함', as_index=False).agg({'금액': 'sum'}) #회전관리선수금
...
df_result5 = df_grouped5.copy() #회전관리선수금
...
output_path5 = r"C:\\dev\\myproj01\\금상동\\결과폴더\\금상동기타엑셀납골함수입합계.xlsx"  # 저장할 파일 경로
df_result5.to_excel(output_path4, index=False)
...
df_result_list5 = df_result5['봉안함'].tolist() #납골함수입
...
#  납골함수입 기타엑셀업로드에 추가 조건에 따라 행 추가
for index, row in total_df10.iterrows():
    if row['봉안함'] not in df_result_list5 and row['차변'] > 0:
        # 첫 번째 행 추가
        df37 = pd.concat([
            df37,
            pd.DataFrame({
...
df37['월일'] = df37['월일'].dt.strftime('%Y-%m-%d')
df37['적요명']=df37['적요명'] + ' && ' + df37['봉안함'].astype(str)
...
columns_to_resize2 = ['a','d','i','h']  # A열부터 D열까지 지정
...
df48 = pd.read_excel(f'C:\\dev\\myproj01\\금상동\\결과폴더\\금상동세무사랑기타업로드+봉안사용료수입-{today}.xlsx')
# 2024-사용료수입이 0보다 큰 행 필터링
df_a_filtered = df_a[df_a['2025-사용료수입'] > 0].copy()
...
df_a57 = df_a[df_a['2025-사용료수입'].notna()]

# '봉안함' 기준으로 그룹화
df_grouped57 = df57.groupby('봉안함', as_index=False).agg({'금액': 'sum'})
df_a_grouped57 = df_a57.groupby('봉안함', as_index=False).agg({'2025-사용료수입': 'sum'})
...
# 금액 차이 계산
#merged_df['금액_차이'] = max(0,(merged_df['2025-사용료수입'].fillna(0) - merged_df['금액'].fillna(0)))
# 금액 차이를 계산할 때 max 함수를 벡터화된 연산으로 변경
merged_df['금액_차이'] = np.maximum(0, merged_df['2025-사용료수입'].fillna(0) - merged_df['금액'].fillna(0))
...
df47['적요명']=merged_df['계약자명'] + ' && ' + merged_df['봉안함'].astype(str)

# df48에서 봉안함-적요명 매핑 dict 만들기
mapping = df48.set_index('봉안함')['적요명'].to_dict()

# df47의 적요명 컬럼 업데이트 (봉안함이 일치할 경우 대체)
df47['적요명'] = df47['봉안함'].map(mapping).fillna(df47['적요명'])
...
# 엑셀 파일 열기
file_path38 = f'C:\\dev\\myproj01\\금상동\\결과폴더\\금상동세무사랑기타업로드+봉안사용료수입추가1225-(9월에합산).xlsx'
wb38= openpyxl.load_workbook(file_path38)
ws38 = wb38.active  # 활성화된 시트를 선택

 # A2 셀 기준으로 틀 고정 (freeze panes)
ws38.freeze_panes = 'A2'
 # 첫 번째 행에 필터 추가
ws38.auto_filter.ref = ws38.dimensions  # ws.dimensions는 사용 중인 셀의 범위를 자동으로 가져옴
    ...
    for sheet in wb38.worksheets:
      for row in range(1, sheet.max_row + 1):
        for col in ['I']:  # E열부터 G열까지
            cell = sheet[f'{col}{row}']
            cell.style = accounting_style
save_path3 = f'C:\\dev\\myproj01\\금상동\\결과폴더\\금상동세무사랑기타업로드-{today}.xlsx' 

wb38.save(save_path3)
print(f"새로운 데이터가 {save_path3} 파일로 저장되었습니다.")   
 ...
 #r'C:\dev\myproj01\금상동\결과폴더\월별결산부+회전관리선수금통합(열조정2024).xlsx'   
file3 = r'C:\dev\myproj01\금상동\결과폴더\월별결산부+회전관리선수금통합(열조정2024).xlsx'

# 두 번째 파일 (추가할 파일)
file4 = f'C:\\dev\\myproj01\\금상동\\결과폴더\\금상동세무사랑기타업로드-{today}.xlsx'

# 파일을 읽어들임
dfs_file4 = pd.read_excel(file4, sheet_name=None)  # 두 번째 파일의 모든 시트를 딕셔너리로 읽기

first_sheet_name = list(dfs_file4.keys())[0]  # 첫 번째 시트 이름 가져오기
df_file4 = dfs_file4[first_sheet_name]  # 첫 번째 시트 데이터 가져오기

# 첫 번째 파일의 기존 엑셀 워크북 불러오기
with pd.ExcelWriter(file3, engine="openpyxl", mode="a") as writer:
    # 기존 시트를 유지하면서 새로운 시트를 추가
    df_file4.to_excel(writer, sheet_name="엑셀기타업로드최종", index=False)
...
# 엑셀 파일의 기존 워크북 불러오기
#with pd.ExcelWriter(file3, engine="openpyxl", mode="a") as writer:
#    for sheet_name, df in dfs_file4.items():  # 두 번째 파일의 시트들 순회
#        df.to_excel(writer, sheet_name="엑셀기타업로드최종", index=False)  # 새로운 시트로 추가

# 시트 순서 조정
wb39 = load_workbook(file3)
sheetnames = wb39.sheetnames

# 새로 추가된 시트 이동
sheet = wb39["엑셀기타업로드최종"]
#wb39.move_sheet(sheet, offset=-len(sheetnames))  # 맨 앞 시트로 이동
wb39.move_sheet(sheet, offset=-11) 

#acc_style.number_format = '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)'

# 엑셀 파일 열기
#file_path38 = f'C:\\dev\\myproj01\\금상동\\결과폴더\\금상동세무사랑기타업로드+봉안사용료수입추가1225-(9월에합산).xlsx'
#wb38= openpyxl.load_workbook(file_path37)
ws39 = wb39.active  # 활성화된 시트를 선택

# 회계 스타일 생성
if 'Accounting' not in wb39.named_styles:  # 이미 존재하면 새로 만들지 않음
    acc_style = NamedStyle(name='Accounting')
    acc_style.number_format = '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)'
    wb39.add_named_style(acc_style)
    
# 모든 시트에 대해 A열부터 D열까지의 너비를 25로 설정
columns_to_resize = ['e','f','g']  # A열부터 D열까지 지정
columns_to_resize2 = ['a','d','h']  # A열부터 D열까지 지정
columns_to_resize3 = ['h']  # A열부터 D열까지 지정

for sheet in wb39.worksheets:
  for row in range(1, sheet.max_row + 1):  
     ## A2 셀 기준 틀 고정
    sheet.freeze_panes = 'A2'
    sheet.auto_filter.ref = sheet.dimensions  # ws.dimensions는 사용 중인 셀의 범위를 자동으로 가져옴
    ...
    for column_letter in columns_to_resize3:
        # 지정된 열(column_letter)의 너비를 25로 설정
...
file5 = r'C:\dev\myproj01\금상동\결과폴더\월별결산부+회전관리선수금통합(맨앞).xlsx'
# 변경사항 저장
wb39.save(file5)


#print(f"'{file4}' 파일의 모든 시트가 '{file3}' 파일에 추가되었습니다.")
print(f"'{file4}' 파일의 첫 번째 시트가 '{file5}' 파일의 맨 앞 시트로 추가되었습니다.")
